...
[tout, yout] = ode45(@(t, sv)coupled_orbit_3d(t, sv,masses, J, N, dplus, dminus), tspan, sv0);
...
cmap = turbo(length(tout));

cb = colorbar
cmap = turbo(length(tout));   % colormap with one color per timestep

for t = 1:length(tout)
    h = animatedline('LineWidth',3);
    h.Color = cmap(t,:);
    for idx = 1:N
        addpoints(h, x_time(1, t, idx)/1000,x_time(2, t, idx)/1000,x_time(3, t, idx)/1000 ); 
    end
    drawnow
end
...
ylabel("y (km)")
zlabel("z (km)")
cb.Label.String = 'Time (s)';  % label colorbar
